const state = {items: [], filter: "all", query: "", editingNoteId: null, editorMode: "write", dirty: false};
const el = {
...
function render() {
  ...
  for (const item of items) {
    const row = document.createElement("button"), badge = document.createElement("span"), body = document.createElement("span"), title = document.createElement("span"), preview = document.createElement("span"), meta = document.createElement("span"), del = document.createElement("button");
...
async function deleteItem(id) { if (!confirm("Delete this item?")) return; try { await api(`/api/v1/items/${id}`, {method: "DELETE"}); toast("Item deleted"); await loadItems(); } catch (error) { toast(error.message, true); } }
...
document.querySelector("#send-file").addEventListener("click", () => el.fileInput.click()); document.querySelector("#new-note").addEventListener("click", () => openNote()); document.querySelector("#close-note").addEventListener("click", () => { if (!state.dirty || window.confirm("Discard unsaved changes?")) el.dialog.close(); }); el.fileInput.addEventListener("change", () => uploadFiles(el.fileInput.files));
