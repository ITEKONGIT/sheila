const state = {items: [], tasks: [], filter: "all", subfilter: "all", query: "", editingNoteId: null, editingTaskId: null, editorMode: "write", dirty: false};
const el = {
  ...
  toast: document.querySelector("#toast"), tasksPanel: document.querySelector("#tasks-panel"),
...
function mediaKind(item) {
  if (["image", "video", "audio"].includes(item.type)) return item.type;
...
function render() {
  ...
  for (const item of items) {
    ...
    row.className = "item"; row.dataset.id = item.id; row.tabIndex = 0; badge.className = "item-icon"; if (kind || item.type === "document") badge.classList.add(`type-${kind || item.type}`); badge.textContent = kind === "image" ? "IMG" : kind === "video" ? "VID" : kind === "audio" ? "AUD" : icon(item); title.className = "item-title"; title.textContent = item.title; preview.className = "item-preview"; preview.textContent = item.type === "note" ? (item.content || "Empty note") : `${item.mediaType || "File"} · ${formatBytes(item.byteSize)}`; body.append(title, preview); meta.className = "item-meta"; meta.textContent = `${formatDate(item.updatedAt)}\n${item.type === "note" ? "Open note" : "Download"}`; meta.style.whiteSpace = "pre-line"; del.type = "button"; del.className = "item-delete"; del.textContent = "\u00d7"; del.dataset.deleteId = item.id; del.setAttribute("aria-label", "Delete"); row.append(badge, body, meta, del);
    if (kind === "video" && item.downloadUrl) { const vid = document.createElement("video"); vid.className = "item-media"; vid.src = item.downloadUrl; vid.controls = true; vid.preload = "metadata"; vid.playsInline = true; body.append(vid); }
    if (kind === "audio" && item.downloadUrl) { const aud = document.createElement("audio"); aud.className = "item-media"; aud.src = item.downloadUrl; aud.controls = true; aud.preload = "metadata"; body.append(aud); }
    if (kind === "image" && item.downloadUrl) { const img = document.createElement("img"); img.className = "item-media"; img.src = item.downloadUrl; img.alt = item.title; img.loading = "lazy"; body.append(img); }
    el.items.append(row);
  }
}
...
async function trashTask(id) { try { await api(`/api/v1/tasks/${id}`, {method: "DELETE"}); toast("Task moved to recycle bin"); await loadTasks(); } catch (error) { toast(error.message, true); } }
async function trashItem(id) { try { await api(`/api/v1/items/${id}`, {method: "DELETE"}); toast("Moved to recycle bin"); await loadItems(); } catch (error) { toast(error.message, true); } }
...
function openInfo(item) {
  ...
  if (kind === "video" && item.downloadUrl) { const vid = document.createElement("video"); vid.src = item.downloadUrl; vid.controls = true; vid.preload = "metadata"; vid.playsInline = true; vid.className = "info-media"; el.infoPreview.append(vid); }
  else if (kind === "audio" && item.downloadUrl) { const aud = document.createElement("audio"); aud.src = item.downloadUrl; aud.controls = true; aud.preload = "metadata"; aud.className = "info-media"; el.infoPreview.append(aud); }
  else if (kind === "image" && item.downloadUrl) { const img = document.createElement("img"); img.src = item.downloadUrl; img.alt = item.title; img.className = "info-media"; el.infoPreview.append(img); }
  else if (item.mediaType === "application/pdf" && item.downloadUrl) { const iframe = document.createElement("iframe"); iframe.src = item.downloadUrl; iframe.className = "info-media"; iframe.style.width = "100%"; iframe.style.height = "400px"; iframe.style.border = "none"; iframe.style.borderRadius = "8px"; el.infoPreview.append(iframe); }
  else if (item.downloadUrl) { const link = document.createElement("a"); link.href = item.downloadUrl; link.textContent = `Download ${item.title}`; link.className = "button secondary"; link.style.display = "inline-block"; link.style.marginTop = "16px"; el.infoPreview.append(link); }
...
document.querySelectorAll(".nav-item[data-filter]").forEach(button => button.addEventListener("click", () => {
  ...
  const labels = {all: ["Everything", "Files and thoughts from every device, in one place."], file: ["Files", "Media and documents stored by the host."], note: ["Notes", "Thoughts that follow you across devices."], tasks: ["Tasks", "Small promises Sheila keeps on schedule."]};
  ...
  const itemView = state.filter !== "tasks"; el.items.hidden = !itemView; el.dropZone.hidden = !itemView; el.tasksPanel.hidden = state.filter !== "tasks";
  if (state.filter === "tasks") loadTasks(); else render();
}));
...
function connectEvents() { const protocol = location.protocol === "https:" ? "wss:" : "ws:", socket = new WebSocket(`${protocol}//${location.host}/api/v1/events`); socket.addEventListener("open", () => { el.connection.textContent = "Host online"; el.connection.classList.add("online"); }); socket.addEventListener("message", event => { try { const message = JSON.parse(event.data); if (message.type === "items.changed") loadItems(); else if (message.type === "tasks.changed") loadTasks(); else if (message.type === "reminder.due") toast(`Reminder: ${message.title}`); } catch {} }); socket.addEventListener("close", () => { el.connection.textContent = "Reconnecting"; el.connection.classList.remove("online"); setTimeout(connectEvents, 2000); }); }
api("/api/v1/system").then(system => { el.storage.textContent = `${formatBytes(system.storage.availableBytes)} free`; }).catch(() => { el.storage.textContent = "Storage unavailable"; }); loadItems(); loadTasks(); connectEvents();
